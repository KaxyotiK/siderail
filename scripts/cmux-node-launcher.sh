...
if [[ -z "${GIT_RAIL_NODE_PATH:-}" ]]; then
  for candidate in /opt/homebrew/bin/node /usr/local/bin/node; do
    if [[ -x "$candidate" ]]; then
      export GIT_RAIL_NODE_PATH="$candidate"
      break
    fi
  done
fi

if [[ -z "${GIT_RAIL_NODE_PATH:-}" && -x "${SHELL:-}" ]]; then
  discovered="$($SHELL -lic 'command -v node 2>/dev/null' 2>/dev/null | /usr/bin/tail -n 1 || true)"
  if [[ "$discovered" == /* && -x "$discovered" ]]; then
    export GIT_RAIL_NODE_PATH="$discovered"
  fi
fi

if [[ "${GIT_RAIL_STAY_OPEN:-0}" == "1" ]]; then
  launch_status=0
  /bin/bash "$script_directory/node-launcher.sh" "$@" || launch_status=$?
  if [[ "$launch_status" -ne 0 ]]; then
    printf 'GitRail exited with status %s.\n' "$launch_status" >&2
  fi
