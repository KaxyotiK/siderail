...
# settling, so a restarted rail that fails within 10 seconds is retried up to 5
# times. A rail that was not restarting, a signal, or a late failure exits.
...
while true; do
  ...
  if [[ "$status" -eq 75 ]]; then
    ...
    attempts=$((attempts + 1))
    printf 'SideRail restart failed with status %s; retrying (%s of 5).\n' "$status" "$attempts" >&2
    /bin/sleep 1
  else
    exit "$status"
  fi
