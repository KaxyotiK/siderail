...
test("startup reconciliation chooses one non-SideRail pane per tab", () => {
...
test("non-Git legacy labels never authorize closing a user pane", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "siderail-auto-open-spoof-"));
...
test("Herdr launch keeps plugin code rooted while passing the selected repository explicitly", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "siderail-auto-open-launch-"));
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  const mock = path.join(root, "herdr-mock");
  const argsFile = path.join(root, "args");
  await fs.writeFile(mock, `#!/bin/sh
...
`);
  ...
  await execFileAsync("bash", ["scripts/open-herdr-panel.sh", "git-tui", "ensure"], {
    ...
    env: {
      ...
      SIDERAIL_WORKSPACE_CWD: selectedRepo,
  ...
  assert.match(calls, new RegExp(`<--env> <SIDERAIL_REPO_ROOT=${selectedRepo.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}>`));
  assert.match(calls, /<--target-pane> <w1:p1>/);
  const stateFiles = await fs.readdir(path.join(root, "cache", "siderail", "panes"));
