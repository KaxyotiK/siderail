...
The action opens SideRail immediately in the current tab. After linking, SideRail
...
Herdr reads the linked checkout directly. To upgrade, close SideRail, update the
...
`.cmux/dock.json` launches `scripts/cmux-siderail.mjs` through a cmux-specific
...
SideRail surface from this checkout. A running configured control records its
process-backed active identity in SideRail's owner-only cache so the launcher
...
SideRail exits; another direct launch reuses that verified terminal.
...
whose live process and terminal-instance identity prove that SideRail owns them,
then unlinks `siderail`. Restart Herdr and remove the checkout only after
`herdr plugin list` reports that SideRail is absent. Optional pane ownership state lives in
`~/.cache/siderail/panes/` and contains only Herdr pane identifiers.
